...
//Global TODOS
// brands selection not working check for all device    
// sorting not working properly check for all device
// Pagination X-Total-count not getting thats way their is a issue. 


const uniquerBrandsObj = new Set(...[data.products.map(el=>(el.category))]);
const uniquBrands = [...uniquerBrandsObj]
function toKebabCase(str) {
    return str
      .toLowerCase()
      .trim()
      .replace(/[^a-z0-9&]+/g, '-') // Replace spaces/special characters (except &)
      .replace(/^-+|-+$/g, '');     // Trim leading/trailing dashes
  }
  
  const result = Array.from(uniquBrands)
